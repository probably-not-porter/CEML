...
class SRSD_GUI:
    ...
    def run(self):
        print('canny only')
        try:
            os.mkdir(self.canny_folder)
        except OSError:
            print ("Creation of the directory %s failed" % self.canny_folder)
        ...
        for filename in os.listdir(self.input_folder):
            if (filename[-4:] == ".jpg") or (filename[-4:] == ".JPG") or (filename[-4:] == ".png"):
                ...
                print(filename)
                time.sleep(1)
                edge_detectors.canny_edge(str(self.input_folder) + "/" + str(filename), self.canny_folder + "/" + "canny_" + str(filename)) #get detection
        print('done')
        self.imagestoprocess.set(self.getImagesToProcess([self.input_folder,self.ndvi_folder,self.canny_folder]))
    ...
    def select_img_folder(self):
        ...
        self.canny_folder = folder_selected + "/canny"
